...
\clearpage
    ...
    \begin{itemize}
        \item \textbf{Retrieve Identifier (`/retrieve-identifier`):} Returns the full unique identifier of the device, specifically the IMEI (15 digits) or the IMEI Software Version (IMEISV, 16 digits).
        \item \textbf{Retrieve Type (`/retrieve-type`):} Returns only the manufacturer, model, and Type Allocation Code (TAC), without exposing the unique serial number of the user's equipment.
        \item \textbf{Retrieve Pseudonym (`/retrieve-ppid`):} Returns a pseudonymised identifier (PPID) that is unique to the API consumer but persistent for the device, enabling privacy-preserving tracking.
    \end{itemize}
    ...
    \begin{itemize}
        \item \textbf{Two-legged Access Token:} The API consumer must explicitly provide a subject identifier in the request body. Supported identifiers include the Phone Number (MSISDN), the IP Address (IPv4 or IPv6), or a Network Access Identifier (NAI). For IPv4, the public port is typically required to handle Carrier-Grade NAT (CGNAT).
        \item \textbf{Three-legged Access Token:} The subject identifier \textbf{MUST NOT} be provided in the request. The device is uniquely identified implicitly from the access token, which is generated based on user consent during the authentication process.
    \end{itemize}
    ...
    \begin{itemize}
        \item \textbf{404 IDENTIFIER\_NOT\_FOUND:} The device identified in the request is not managed by the API provider.
        \item \textbf{422 SERVICE\_NOT\_APPLICABLE:} A device identifier cannot be provided for the target (e.g., the phone number identifies a landline).
        \item \textbf{422 MISSING\_IDENTIFIER:} The subject cannot be identified from the access token, and no device object was provided in the request.
        \item \textbf{422 UNNECESSARY\_IDENTIFIER:} A device object was provided in the request, but the subject was already identified via a 3-legged access token.
    \end{itemize}
    
    \paragraph{Use Cases}
    The ability to retrieve trusted device information supports several business scenarios:
    \begin{itemize}
        \item \textbf{Security and Fraud Prevention:} Establishing that a customer is not using a device they previously reported as lost or stolen, or verifying device possession for authentication.
        \item \textbf{Insurance:} Automatically populating device details (make, model, IMEI) when a customer wishes to insure a device, preventing input errors.
        \item \textbf{Service Optimization:} Adapting content delivery or software versions based on the specific device model or OS type detected by the network.
    \end{itemize}
    ...
    \begin{itemize}
        \item \textbf{Retrieve Date (`/retrieve-date`):} Provides the timestamp of the latest SIM swap event. If no swap has been performed within the operator's data retention period, the API may return the SIM activation date. If regulations or privacy policies prevent returning the date, a \texttt{null} value is returned.
        \item \textbf{Check (`/check`):} Validates whether a SIM swap has occurred during a past period defined by the \texttt{maxAge} attribute (e.g., the last 24 hours). This allows for a boolean verification without exposing the exact timestamp.
    \end{itemize}
    ...
    \begin{itemize}
        \item \textbf{Two-legged Access Token:} The phone number MUST be explicitly provided in the request body (e.g., via a \texttt{phoneNumber} identifier).
        \item \textbf{Three-legged Access Token:} The phone number MUST NOT be provided in the request body. The subject is uniquely identified implicitly from the access token, which is associated with the user during the authentication process.
    \end{itemize}
    ...
    \begin{itemize}
        \item \textbf{400 OUT\_OF\_RANGE:} The \texttt{maxAge} parameter in a check request exceeds the operator's allowed monitoring period (e.g., 2400 hours) or local regulations.
        \item \textbf{422 MISSING\_IDENTIFIER:} The phone number cannot be identified from the access token and was not included in the request body.
        \item \textbf{422 UNNECESSARY\_IDENTIFIER:} A phone number was provided in the request body, but the subject was already identified via a 3-legged access token.
        \item \textbf{501 NOT\_IMPLEMENTED:} Returned if a specific functionality is explicitly not supported by the implementation.
    \end{itemize}
    
    \paragraph{Use Cases}
    The ability to verify the integrity of the SIM-MSISDN association enables several critical security applications:
    \begin{itemize}
        \item \textbf{Fraud Prevention in Banking:} Financial institutions can query the API during suspicious transactions to ensure the user's SIM has not been compromised recently.
        \item \textbf{Password Reset Protection:} Online service providers (e-commerce, social networks) can verify SIM status before sending SMS OTPs for password recovery to prevent account takeover.
        \item \textbf{Non-Automated Actions:} Supporting call center experts by confirming the stability of the user's line before performing sensitive operations manually.
    \end{itemize}
    ...
    \begin{itemize}
        \item \textbf{Max Age (`maxAge`):} Specifies the maximum acceptable age of the location information in seconds. A value of `0` requests a fresh calculation. If omitted, any cached age is acceptable.
        \item \textbf{Max Surface (`maxSurface`):} Specifies the maximum acceptable surface area in square meters. This controls the accuracy; if the network cannot locate the device within this limit, it returns an error.
    \end{itemize}
    ...
    \begin{itemize}
        \item \textbf{Two-legged Access Token:} The subject must be identified from an optional `device` object in the request body (e.g., PhoneNumber, IPv4, IPv6).
        \item \textbf{Three-legged Access Token:} The `device` object MUST NOT be provided. The subject is uniquely identified from the access token itself, ensuring transparency and compliance with user-centric privacy-by-design principles.
    \end{itemize}
    ...
    \begin{itemize}
        \item \textbf{422 UNABLE\_TO\_LOCATE:} The system cannot determine the device's location.
        \item \textbf{422 UNABLE\_TO\_FULFILL\_MAX\_AGE:} The system cannot provide location data fresher than the requested `maxAge`.
        \item \textbf{422 UNABLE\_TO\_FULFILL\_MAX\_SURFACE:} The system cannot provide an area within the requested `maxSurface` limit.
        \item \textbf{404 IDENTIFIER\_NOT\_FOUND:} The provided identifier cannot be matched to a known device.
    \end{itemize}
    
    \paragraph{Use Cases}
    Trusted location data supports various critical scenarios:
    \begin{itemize}
        \item \textbf{Fraud Prevention:} Verifying if a financial transaction or high-value purchase originates from a location consistent with the user's expected region.
        \item \textbf{Contextual Advertising:} Triggering promotions only when verifying the device is physically within a specific area of interest (e.g., a shopping district).
        \item \textbf{Smart Mobility:} Obtaining the location of shared vehicles (bikes/scooters) to guarantee they are rented or parked correctly.
        \item \textbf{Emergency Services:} Providing accurate location information to responders when GPS is unavailable.
    \end{itemize}
    ...
    \begin{itemize}
        \item \textbf{TRUE:} The network's estimation of the device's location is fully contained within the requested area.
        \item \textbf{FALSE:} The device location does not overlap with the requested area at all.
        \item \textbf{PARTIAL:} The device location partially overlaps or fully contains the requested area (because the network area is larger). In this case, a `matchRate` percentage may be included to estimate the likelihood of the match.
    \end{itemize}
    ...
    \begin{itemize}
        \item \textbf{Two-legged Access Token:} The subject must be explicitly identified via the `device` object (e.g., PhoneNumber, IPv4) in the request body.
        \item \textbf{Three-legged Access Token:} The `device` object \textbf{MUST NOT} be provided. The subject is uniquely identified from the access token, which is mandatory when users exercise rights such as opt-in/opt-out for personal data processing.
    \end{itemize}
    ...
    \begin{itemize}
        \item \textbf{422 AREA\_NOT\_COVERED:} The provided area is outside the operator's coverage.
        \item \textbf{422 INVALID\_AREA:} Legal restrictions or implementation issues force the operator to reject the area (e.g., the requested radius is too small for privacy compliance).
        \item \textbf{422 UNABLE\_TO\_LOCATE:} The network cannot locate the device.
        \item \textbf{422 UNABLE\_TO\_FULFILL\_MAX\_AGE:} Fresh location data cannot be obtained within the requested `maxAge`.
    \end{itemize}
    
    \paragraph{Use Cases}
    The verification model supports high-security and geo-compliance scenarios:
    \begin{itemize}
        \item \textbf{FinTech Fraud Prevention:} Banks can verify if a user attempting a transaction in a specific city is physically present there, detecting card cloning or remote attacks.
        \item \textbf{Geo-blocking and Content Rights:} Media services can verify if a user is in an authorized region for streaming content, preventing VPN circumvention.
        \item \textbf{Smart Mobility:} Confirming that a rented vehicle (bike/scooter) is located within a valid parking zone before ending the rental session.
    \end{itemize}
    ...
    \begin{itemize}
        \item \textbf{Two-legged Access Token:} The consumer must provide the device identifier (e.g., Phone Number, IPv4 address) in the query parameters or request body.
        \item \textbf{Three-legged Access Token:} The device identifier is derived implicitly from the token (bound to the user's consent), and explicit identifiers in the request are omitted.
    \end{itemize}
    ...
    \begin{itemize}
        \item \textbf{401 UNAUTHENTICATED:} The access token is missing or invalid.
        \item \textbf{403 PERMISSION\_DENIED:} The client does not have the scope to read QoS profiles.
        \item \textbf{404 IDENTIFIER\_NOT\_FOUND:} The requested device or subscription could not be found.
        \item \textbf{501 NOT\_IMPLEMENTED:} The discovery functionality is not supported by the specific operator implementation.
    \end{itemize}
    
    \paragraph{Use Cases}
    The discovery capability enables adaptive application behavior:
    \begin{itemize}
        \item \textbf{Pre-flight Validation:} A cloud gaming application can check if the `QOS\_L` (Low Latency) profile is available before starting a session. If unavailable (e.g., due to the user's plan or roaming status), the app can adjust settings or notify the user.
        \item \textbf{Roaming Adaptation:} Since QoS profile names and availability may vary between roaming partners, applications can use this API to dynamically discover the valid profiles in the visited network.
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{QoS Profile:} The desired service level (e.g., `QOS\_L` for Low Latency), which determines the specific network parameters applied.
        \item \textbf{Duration:} The requested lifetime of the session in seconds.
    \end{itemize}
    ...
    \begin{itemize}
        \item \textbf{Two-legged Access Token:} The consumer explicitly provides the device identifier. In QoD, identifying the device by \textbf{IPv4/IPv6 address} is often preferred over the Phone Number, as it allows the network to apply QoS policies to the specific data flow associated with that IP connection.
        \item \textbf{Three-legged Access Token:} The subject is identified from the token. However, for precise traffic prioritization (e.g., applying QoS only to the gaming traffic and not background downloads), the application source IP might still be required in the session creation context.
    \end{itemize}
    ...
    \begin{itemize}
        \item \textbf{409 CONFLICT:} A session already exists for this device, or the request conflicts with an existing policy.
        \item \textbf{422 QOS\_PROFILE\_NOT\_FOUND:} The requested profile name (e.g., `QOS\_L`) is not valid or available for this user.
        \item \textbf{422 UNAVAILABLE:} The network cannot fulfill the QoS request at the current location due to congestion or lack of resources.
        \item \textbf{501 NOT\_IMPLEMENTED:} The requested duration or profile features are not supported by the operator.
    \end{itemize}
    
    \paragraph{Use Cases}
    QoD enables a new class of network-aware applications:
    \begin{itemize}
        \item \textbf{Real-Time Immersion (XR/Gaming):} Requesting low latency to minimize motion-to-photon delay in VR headsets or cloud gaming, preventing motion sickness.
        \item \textbf{Remote Operations (Tele-driving):} Ensuring command signals for drones or industrial robots arrive within a strict time window to guarantee safety.
        \item \textbf{Critical Communications:} Prioritizing video streams or control data for emergency response devices in congested areas.
    \end{itemize}
